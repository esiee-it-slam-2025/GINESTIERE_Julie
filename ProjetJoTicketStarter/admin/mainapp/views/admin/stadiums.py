...
class StadiumsEditView(TemplateView):
    template_name = 'stadiums_edit.html'

    def get(self, request):
        if request.user.is_superuser:
            StadiumFormSet = modelformset_factory(Stadium, form=StadiumForm, extra=0)
            formset = StadiumFormSet(queryset=Stadium.objects.all())
    ...
    def post(self, request):
        if request.user.is_superuser:
            StadiumFormSet = modelformset_factory(Stadium, form=StadiumForm, extra=0)
            formset = StadiumFormSet(request.POST)
            if formset.is_valid():
                formset.save()
